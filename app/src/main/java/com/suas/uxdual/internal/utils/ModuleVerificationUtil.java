package com.suas.uxdual.internal.utils;
...
public class ModuleVerificationUtil {
    ...
    public static boolean isCameraModuleAvailable(int cameraid) {
        if(!isProductModuleAvailable()) return false;
        List<Camera> cameras = DJISampleApplication.getProductInstance().getCameras();
        if(cameras == null) return false;
        return isProductModuleAvailable() && (null != DJISampleApplication.getProductInstance().getCameras().get(cameraid));
    }
